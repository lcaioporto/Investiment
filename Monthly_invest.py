...
def output_message_box(title, message):
...
def interface():
    ...
    def submit(output):
        ...
        def clear_entries():
            initial_money_entry.delete(0, tk.END)
            monthly_invest_entry.delete(0, tk.END)
            tax_entry.delete(0, tk.END)
            years_entry.delete(0, tk.END)
            money_entry.delete(0, tk.END)

        if not (initial_money and monthly_invest and tax and years and expt_money):
            messagebox.showinfo(
            ...
            return
        try:
            initial_money, monthly_invest, tax, years, expt_money = [float(var) for var in [initial_money, monthly_invest, tax, years, expt_money]]
        except Exception:
            messagebox.showinfo(
                title="Error!",
...
def main():
    ...
    output_message_box(title="INVESTMENT REPORT", message=inv_report)
